...
| `SYT-036` | Home hover stale card lifecycle regression | Runner | Needs Review | `swarm/syt-036-home-hover-stuck-lifecycle` | #36 native Home hover lifecycle cleanup and fixture regression | serial-required | `SYT-031`, `SYT-010G` | high, live YouTube SPA preview lifecycle | `docs/swarm/handoffs/SYT-036.md` | pending |
...
| `SYT-036` | pending | Not started | Prefer yes before merge unless waived with live evidence | `Human QA passed/failed for SYT-036: <notes>` |
...
- 2026-06-10: User reported Home cards retaining opaque hover backgrounds and hover autoplay failing after refresh -> watch page -> SPA Home navigation. Controller opened #36 and implemented `SYT-036` on `swarm/syt-036-home-hover-stuck-lifecycle`; route review after validation/PR.
